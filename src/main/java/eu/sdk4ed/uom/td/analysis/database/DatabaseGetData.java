...
public class DatabaseGetData 
{
	...
	public ArrayList<Double> getCouplingCohesion(String fileName, int version)
	{
		...
		query = "SELECT DISTINCT coupling, cohesion FROM cMetrics WHERE class_name = (?) AND version = (?) ";
		try 
		{
	...
	// new function  
	public ArrayList<Double> getLocForArtifactPerVersion(String className, int version, String language, String projectName)
	{
		Connection conn = DatabaseConnection.getConnection();
		PreparedStatement pstm = null;
		ResultSet resultSet = null;
		...
		if (language.equals("Java"))
			tableName = "javaMetrics";
		else 
			tableName = "cMetrics";
		
		query = "SELECT DISTINCT loc, version FROM " + tableName + " WHERE class_name LIKE (?) AND version <= (?) AND project_name LIKE (?) ORDER BY version ASC";
		try 
		{
			pstm = conn.prepareStatement(query);
			pstm.setString(1, className);
